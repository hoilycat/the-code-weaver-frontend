...
핵심 계산과 UI 흐름은 로컬에서 동작하며 계산 테스트 6개를 통과했습니다. Hosted YIE 연결과 근거 출처 상세 화면은 다음 검증 범위입니다.`,
  },
  {
    ...
    preferEditorialMedia: true,
    snapshot: "/media/weldvision-qa/hero-qa-dashboard.jpg",
...
실시간 제품 흐름과 데모는 구현했지만 자세 감지 정확도는 아직 정량 평가 전입니다.`,
  },
  {
    ...
    editorialLabel: "Team · Visual Design Lead",
    disciplines: ["AI", "Full-stack"],
    ...
    proofSummary: "페이지별 프론트엔드·백엔드 분담, 공동 작명·다수결 합의·교차 QA는 팀 협업으로 구분하고, 직접 맡은 디자인 제작·담당 화면·DB 구조 검토를 작업물과 함께 기록합니다.",
    implemented: [
      ...
      "로고·앱 아이콘과 3초 MP4 스플래시 직접 제작, 팀 검토 후 확정",
      "사진 추가·EXIF/GPS 전처리와 이미지 로딩·생성 대기 화면 구현",
      "백엔드 팀과 데이터베이스 구조·저장 항목 검토 및 구성 방향 논의",
    ...
    evidence: [
      ...
      "브랜드 보드·스토리보드·프로토타입",
      "Dark·Light 스플래시 영상",
      "앱명·팀명 공동 결정과 로고·스플래시 팀 검토 과정",
      "페이지별 프론트엔드·백엔드 분담과 정기 회의·DB 구조 논의·교차 QA 명시",
    ],
    nextValidation: ["릴리스 빌드 QA와 팀 인수인계 문서 정리"],
    resources: [
      { label: "Team Repository", url: "https://github.com/ryuh2929/SceneDiary" },
    ],
    description: `SceneDiary는 여행 사진을 AI가 해석해 날짜별 일기로 구성하는 팀 프로젝트입니다. 저는 Visual Design Lead로서 브랜드와 스플래시, 소개 이미지, A1 학술 포스터, 19장 발표자료의 시각 체계를 설계하고 최종 제작과 문장 QA를 맡았습니다. 프론트엔드에서는 사진 추가와 이미지 로딩·생성 대기 화면을 구현했으며, 백엔드 팀과는 데이터베이스 구조를 검토했습니다. 화면 연결과 교차 QA는 담당 팀원들과 함께 진행했습니다.

[Project Notes]

Role
Visual Direction, Poster Design Lead, Information Design, Presentation Design, Retouching, Copy Editing, Frontend Screens, Database Review, Cross QA

Visual Direction
GPT로 만든 이미지는 초안 소재로 사용했습니다. 바다와 구름 영역을 직접 확장하고 연결부를 리터칭한 뒤, 로고·카피·실제 앱 화면을 편집해 Hero와 기능 소개 이미지를 완성했습니다. 앱 이름과 팀 이름은 함께 정했으며, 직접 제작한 로고와 스플래시는 팀 검토와 합의를 거쳐 확정했습니다.
...
Collaboration
팀장이 페이지를 고르게 분담해 팀원 모두 각자 맡은 화면의 프론트엔드와 백엔드 작업에 참여했습니다. 정기 회의에서 개발 현황과 수정 사항을 공유하고 주요 기획·디자인 방향은 다수결로 합의했습니다. 앱 이름과 팀 이름은 함께 정했으며, 제가 만든 로고와 스플래시는 팀 검토를 거쳐 확정했습니다. 저는 사진 추가와 이미지 로딩·생성 대기 화면을 맡고, 백엔드 팀과 데이터베이스 구조 및 저장 항목을 논의했습니다. GraphRAG 고도화, 문체 모델, 편집 기능은 각 담당 팀원이 주도했으며, 화면 연결과 사용자 흐름은 팀원들과 함께 점검하고 교차 QA했습니다. 포스터는 AI 초안 → 학술 포스터 형식 피드백 → 화면과 정보 구조 전면 재설계 → 팀 중간 확인 → 문장·오탈자 최종 검수 순서로 제작했습니다. 발표자료는 팀원 초안 → 시각 자료 재구성과 기술 설명 보강 → 팀 공동 첨삭 → 팀원의 후속 편집 → 최종 QA 순서로 완성했습니다.

Project Type
Team Project, Visual Design Lead, Full-stack, AI Storytelling

Results & Limitations
페이지별 프론트엔드·백엔드 분담, 공동 작명·다수결 의사결정·화면 연결·교차 QA와 팀원이 주도한 GraphRAG·문체 모델·편집 기능은 협업 과정으로 구분합니다. 제가 직접 수행한 포스터 재설계·이미지 리터칭·문장 교정·담당 화면 구현과 데이터베이스 구조 검토 범위는 작업물과 함께 공개합니다.`,
  },
  {
    id: 5,
    priority: 8,
...
디자인과 프론트엔드 사용자 흐름을 구현했습니다. 백엔드와 AI 검색 엔진은 팀원의 기여이며 제 구현 범위와 구분해 설명합니다.`,
  },
  {
    ...
    period: "2025–2026",
    cardSummary: "Selenium으로 9,000+ 문화유산 자료를 수집하고 21회 정제해 5,259개 분석 데이터로 만든 시각화",
    proofSummary: "기계적 분류의 한계를 인정하고 Human-in-the-loop 방식으로 분류 기준과 예외를 반복 검토했습니다.",
    implemented: ["Selenium 기반 국립중앙박물관 공개 자료 스크래핑", "V1–V21 반복 정제", "7개 대분류 규칙", "Tableau와 Inkscape 대시보드"],
    evidence: ["GitHub에 수집 노트북 공개", "기타 1,000개→288개 축소", "최종 정제 5,259개", "Tableau 공개 대시보드"],
    nextValidation: [],
    resources: [
    ...
    description: "Selenium으로 국립중앙박물관 공개 페이지를 지역과 페이지별로 순회해 문화유산 자료 9,000개 이상을 직접 수집했습니다. 이후 중복·결측·분류 예외를 21차례 정제해 5,259개 분석 데이터로 만들었습니다. 일곱 개 대분류와 Human-in-the-loop 검수를 적용한 결과, 분류가 어려운 ‘기타’ 항목을 1,000개에서 288개로 줄였고 유물의 발견지와 소장 박물관 흐름을 Tableau로 시각화했습니다.",
  },
  {
    id: 6,
    priority: 10,
